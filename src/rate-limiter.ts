 ...
 *
 * @returns A new RateLimiter with default settings (10 msg/sec)
 */
...
/**
 * Throttle helper that wraps a function with rate limiting
 *
...
export async function withRateLimit<T>(
  ...
  limiter.check(agentId);
  return fn();
}

/**
 * Create a rate limit error response object
 *
...
export function createRateLimitResponse(retryAfterMs?: number) {
  return {
    ...
    error: retryAfterMs !== undefined
      ? `Rate limit exceeded: max 10 messages per second. Retry after ${retryAfterMs}ms.`
